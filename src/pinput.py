...
class PlayerInput:
    ...
    def iterate(self):
        ...
        UPKEYS = _key_map[K_UP] or _key_map[K_k] or _key_map[K_SPACE] or bool(_mouse_click[0]) or _key_map[K_w]
        ...
        if UPKEYS and (not self.key_up[0]): self.key_up[1] = True
        if _mouse_click[0] and (not self.mouse_click_left[1]): self.mouse_click_left[1] = True
        if _mouse_click[1] and (not self.mouse_click_middle[1]): self.mouse_click_middle[1] = True
        if _mouse_click[2] and (not self.mouse_click_right[1]): self.mouse_click_right[1] = True

        # Limpar/preencher a lista de teclas seguradas
        self.key_up[0] = UPKEYS
        self.mouse_click_left[0], self.mouse_click_middle[0], self.mouse_click_right[0] = [bool(x) for x in _mouse_click]
