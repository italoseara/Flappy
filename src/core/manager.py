...
class GameManager:
    def __init__(self, title: str, win_size: Vector2):
        ...
        self.screen = PygameSurface(pygame.display.set_mode(self._win_size, vsync=True))
    ...
    def fill_screen(self, color):
        self.screen.into_pygame().fill(color)

    def render(self, render: Render):
        render.draw_to(self.screen.into_pygame())

    def blit(self, blittable: Blittable, pos: Vector2):
        blittable.draw_to(self.screen.into_pygame(), pos)

    def render_rect(self, rect, line_color, line_size):
        pygame.draw.rect(self.screen.into_pygame(), line_color, rect, line_size)
