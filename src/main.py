...
def main(argv):
    ...
    # TODO: Usar sistema de argv

    # Iniciar o pygame
    pygame.init()

    # Criar e iniciar o jogo
    game = FlappyGame(game_speed=3,
                      debug=False)
    game.runtime()
...
class FlappyGame(Game):
    ...
    def _load_resources(self):
        ...
        # Fontes utilizadas
        self.score_font = pygame.font.SysFont('Ubuntu Mono, Cascadia Code, Consolas, Fixedsys', 20)
    ...
    def runtime(self):
        ...
        floor_tiles, bg_tiles = [], [] # PATCH: Criar as listas de tiles antes de preenchê-las para poder fazer o código mais abaixo funcionar.
        ...
        while running:
            ...
            if self.state == 1:
                ...
                if var.pipe_spawn_counter <= 0:
                    ...
                    spacing = 130
                    bottom_pipe_height = height + image_size(self.img['tube_top'])[1] + spacing

                    # Criar o cano de cima
                    new_pipe_top = Pipe((WINSIZE[0], height), [self.img['tube_top']], self.speed)
                    pipes_list.append(new_pipe_top)

                    # Criar o cano de baixo
                    new_pipe_bottom = Pipe((WINSIZE[0], bottom_pipe_height), [self.img['tube_bottom']], self.speed)
                ...
                for (pipe_index, pipe) in enumerate(pipes_list):
                    ...
                    if gameobject_hitbox(pipe).colliderect(gameobject_hitbox(player)):
                        self.state = 2
                        player.speed.y = -8

                    # TODO: Melhorar esse bloco (está altamente instável)
                    if player.y == pipe.y:
                        self.score += 1
            ...
            if self.debug:
                ...
                drect(screen, HITBOX_COLOR, (0, self.ground_pos, *WINSIZE), HITBOX_WIDTH)
            ...
            # Tela de Game over
            if self.state == 2:
                screen.blit(self.img['game_over'], (100, 0)) # TODO: Cálculo de posicionamento melhor
